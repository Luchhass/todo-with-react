...
export default function TodoApp() {
    ...
    function toggleTodoCompletion(id) {
        ...
        setTodos(todos.map(todo => todo.id === id ? { ...todo, isComplated: !todo.isComplated } : todo));
    }
    ...
    return (
        ...
        <>
            ...
            <div className="todoListSection">
                <Todolist todos={getFilteredTodos()} deleteTodo={deleteTodo} toggleTodoCompletion={toggleTodoCompletion} />
            </div>
...
function TodoForm({ appendTodo }) {
    ...
    return (
        ...
        <form onSubmit={handleSubmit} autoComplete="off">
            <button>add</button>
            <input required name="task" type="text" placeholder="Create a new todo…" />
        </form>
    );
}
...
function Todolist({ todos, deleteTodo, toggleTodoCompletion }) {
    return (
        ...
        <ul className="todolist">
            {todos.map(x => (
                <TodoItem key={x.id} id={x.id} task={x.task} isComplated={x.isComplated} deleteTodo={deleteTodo} toggleTodoCompletion={toggleTodoCompletion} />
            ))}
        </ul>
    );
}
        ...
        <li>
            <div className="todoTask">
                <label className="todoCheck">
                    <input type="checkbox" onChange={handleCheckboxChange} checked={isComplated} />
